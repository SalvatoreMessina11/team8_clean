...
SURFACE_RE = re.compile(
    r"GLD_(\d{4}-\d{2}-\d{2})_eligible_(adaptive|historical|full)_surface\.csv$"
)
...
def discover_surface_files(*dirs: Path) -> dict[pd.Timestamp, tuple[str, Path]]:
    ...
    for directory in dirs:
        ...
        for path in directory.glob("GLD_*_eligible_*_surface.csv"):
            ...
            if current is None or PRIORITY[kind] > PRIORITY[current[0]]:
                best[date] = (kind, path)
...
def inspect_surface(
    ...
    bad_required = int(
        (~np.isfinite(required_matrix)).any(axis=1).sum()
    )

    below_min_dte = int((dte < float(min_dte)).fillna(True).sum())
...
def main():
    ...
    surfaces = discover_surface_files(full_dir, sparse_dir)
