...
3. If an origin surface has >= 64 eligible observations, calibration uses the
   fixed 8x8 Chebyshev-Chebyshev (CC) sample of ACTUAL observations.
4. If an origin surface has < 64 eligible observations, calibration uses all
   actual eligible observations. No interpolation creates synthetic options.
...
The script prefers, for duplicate dates:
    adaptive > historical > full
...
def discover_surfaces(
    ...
    """Discover and de-duplicate historical surfaces.

    For the same date, adaptive is preferred to historical, which is preferred
    to the legacy full-surface file.
    """
    best: dict[pd.Timestamp, SurfaceRef] = {}
    ...
    for directory in (Path(sparse_dir), Path(dense_dir)):
        ...
        for path in directory.glob("GLD_*_eligible_*_surface.csv"):
            ...
            candidate = SurfaceRef(date=date, kind=kind, path=path)
            current = best.get(date)

            if (
                current is None
                or SURFACE_PRIORITY[kind] > SURFACE_PRIORITY[current.kind]
            ):
                best[date] = candidate
...
def main() -> None:
    ...
    print("Sparse rule                : all actual eligible observations")
    ...
    forecasts, date_metrics, summary = run_rolling(
        ...
        min_origin_points=int(args.min_origin_points),
        min_origin_expiries=int(args.min_origin_expiries),
